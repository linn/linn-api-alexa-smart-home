...
import type { IAlexaRequest, IAlexaResponse, IAlexaResponseContext, IChannelRequestPayload } from '../models/Alexa';
    ...
    async handle(request: IAlexaRequest<IChannelRequestPayload>): Promise<IAlexaResponse<{}>> {
        switch (request.directive.header.name) {
            case 'ChangeChannel': {
                ...
                if (
                    !pinId &&
                    request.directive.payload.channelMetadata &&
                    request.directive.payload.channelMetadata.name
                ) {
                    const pinMatch = request.directive.payload.channelMetadata.name.match(/\d+/);
                    if (pinMatch) {
                ...
                await this.facade.invokeDevicePin(
