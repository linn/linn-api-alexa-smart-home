...
import type { IAlexaRequest, IAlexaResponse, ISpeakerRequestPayload } from '../models/Alexa';
            ...
            case 'AdjustVolume':
                if (request.directive.payload.volumeDefault) {
                    if (request.directive.payload.volume > 0) {
                        await this.facade.adjustVolume(
                    ...
                    await this.facade.adjustVolume(
                        request.directive.endpoint.endpointId,
                        request.directive.payload.volume,
                        request.directive.endpoint.scope.token
                    );
                }
                break;
            case 'SetMute':
                await this.facade.setMute(
                    request.directive.endpoint.endpointId,
                    request.directive.payload.mute,
                    request.directive.endpoint.scope.token
                );
                break;
            case 'SetVolume':
