...
if jq -e '.FunctionError' /tmp/smoke-invoke.json > /dev/null 2>&1; then
  echo "SMOKE FAILED: the function errored" >&2
  cat "$OUT" >&2
  exit 1
fi
...
check '.event.header.namespace' 'Alexa'
check '.event.header.name' 'ErrorResponse'
check '.event.header.payloadVersion' '3'
...
check '.event.header.correlationToken' 'smoke-correlation'
check '.event.endpoint.endpointId' 'smoke-test-device'
check '.event.payload.type' 'INVALID_AUTHORIZATION_CREDENTIAL'
